...
namespace Com.AugustCellars.CoAP.Examples
{
    ...
    class ExampleClient
    {
        ...
        {
            string Name = args[0];
            StopWatch st = new StopWatch("C:\\CoAP\\performanceCoAP_Client_CSharp_" + Name + ".log");
            ...
            int verb = 1;//--------------------------------------------------change request type
            ...
            while (true)
            {
                ...
                string deviceName = "A";
                string[] arrRequestSize = { "10", "100", "1000", "10000" };//---------------------------------------change request size
                string requestDataSize = GetRandom(arrRequestSize);
                string uri = "coap://" + ipaddress + ":" + port + "/Devices" ; //coap://[::1]/temperature

                if (verb == 0)
                {
                                   
                    Request requestGet = new Request(Method.GET);
                    requestGet.URI = new Uri(uri);
                    requestGet.UriQuery = "/" + deviceName + "/" + requestDataSize;
                    string host = requestGet.URI.Host;
                   // requestGet.Payload = payloadByte;

                    int size = requestGet.ToString().Length;
                    st.Start();
                    requestGet.Send();
                    response = requestGet.WaitForResponse();                 
                    st.Stop();
                    st.WriteLogClient("GET",deviceName, size.ToString() , Name, response.StatusCode.ToString());
                ...
                if(verb == 1)
                {
                    ...
                    Request requestPOST = new Request(Method.POST);
                    requestPOST.URI = new Uri(uri);
                    requestPOST.UriQuery = "/" + deviceName + "/" + requestDataSize;

                    st.Start();
                    requestPOST.PayloadString = payload;

                    requestPOST.Send();
                    response = requestPOST.WaitForResponse();
                    st.Stop();
                    st.WriteLogClient("POST", deviceName, requestDataSize, Name, response.StatusCode.ToString());
                    Console.WriteLine(response.ToString());
                }
